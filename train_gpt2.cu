...
__global__ void
adamw_kernel2(float *params_memory, float *grads_memory, float *m_memory, float *v_memory, long num_parameters,
              float learning_rate, float beta1, float beta2, float beta1_correction, float beta2_correction, float eps,
              float weight_decay) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
...
void matmul_forward_cublas(float *out,
    ...
    cublasCheck(
            cublasSgemm(cublas_handle, CUBLAS_OP_T, CUBLAS_OP_N, OC, B * T, C, &alpha, weight, C, inp, C, &beta, out,
                        OC));
...
void matmul_forward_cublaslt(float *out,
    ...
    cublasCheck(cublasLtMatmulDescSetAttribute(operationDesc, CUBLASLT_MATMUL_DESC_TRANSA, &opTranspose,
                                               sizeof(opTranspose)));
...
void gpt2_build_from_checkpoint(GPT2 *model, const char *checkpoint_path) {
    ...
    cudaCheck(cudaMemcpy(model->params_memory, params_memory_cpu, num_parameters * sizeof(float),
...
void gpt2_zero_grad(GPT2 *model) {
    if (model->grads_acts_memory != NULL) {
        cudaCheck(cudaMemset(model->grads_acts_memory, 0, model->num_activations * sizeof(float)));
    }
...
void gpt2_backward(GPT2 *model) {
    ...
    for (int l = L - 1; l >= 0; l--) {
        ...
        layernorm_backward(dl_residual2, dl_ln2w, dl_ln2b, dl_ln2, l_residual2, l_ln2w, l_ln2_mean, l_ln2_rstd, B, T,
                           C);
        residual_backward(dresidual, dl_attproj, dl_residual2, B * T * C);
        matmul_backward(dl_atty, dl_attprojw, dl_attprojb, dl_attproj, l_atty, l_attprojw, B, T, C, C);
        attention_backward(dl_qkv, dl_qkvr, dl_preatt, dl_att, dl_v_accum, dl_atty, l_qkv, l_qkvr, l_preatt, l_att,
                           l_v_accum, B, T, C, NH);
...
void gpt2_update(GPT2 *model, float learning_rate, float beta1, float beta2, float eps, float weight_decay, int t) {
    ...
    adamw_kernel2<<<num_blocks, block_size>>>(model->params_memory, model->grads_memory, model->m_memory,
                                              model->v_memory,
    ...
    for (int step = 0; step <= 40; step++) {
        ...
        if (step > 0 && step % 20 == 0) {
            ...
            for (int t = 1; t < genT; t++) {
                ...
                cudaCheck(
                        cudaMemcpy(cpu_probs, probs, model.config.vocab_size * sizeof(float), cudaMemcpyDeviceToHost));
